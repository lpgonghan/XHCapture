...
//  XHVideoPlayer
//
//  Created by Xuehan Gong on 2017/10/23.
...
public class SwiftCountDownTimer {
    public typealias CountDownEventHandler = (SwiftCountDownTimer, _ leftTimes: Int, _ consumeTimes: Int) -> Void
    private let internalTimer: SwiftTimer
    private var leftTimes: Int
    private let originalTimes: Int
    private let handler: CountDownEventHandler
    public init(interval: DispatchTimeInterval, times: Int,queue: DispatchQueue = .main , handler:  @escaping CountDownEventHandler ) {
        ...
        self.internalTimer.rescheduleHandler { [weak self]  swiftTimer in
            if let strongSelf = self {
                if strongSelf.leftTimes > 0 {
                    ...
                    strongSelf.handler(strongSelf, strongSelf.leftTimes, strongSelf.originalTimes - strongSelf.leftTimes)
                } else {
